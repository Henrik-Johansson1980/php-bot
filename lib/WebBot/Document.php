...
class Document
{
	 ...
	 * @return boolean
	 */
	public function test($value, $caseInsensitive = true)
	{
		if (preg_match('#^\/.*\/$#', $value)) // regex  pattern
		{
			return preg_match(
				$value . 'Usm' . ($caseInsensitive ? 'i' : ''),
				$this->getHttpResponse()->getBody()
			);
		} else // no regex, use string position
		{
			return call_user_func(($caseInsensitive ? 'stripos' : 'strpos'),
				$this->getHttpResponse()->getBody(),
				$value
			) !== false;
		}
	...
	public function find($value, $read_length_or_str = 0, $caseInsensitive = true)
	{
			if($this->test($value, $caseInsensitive))
			{
					if(preg_match('#^\/.*\/$#', $value)) // regex pattern
					{
							preg_match_all($value . 'Usm' . ( $caseInsensitive ? 'i' : '' ),
					...
					{
							$pos = call_user_func(( $caseInsensitive ? 'stripos' : 'strpos' ),
							...
							if(is_string($read_length_or_str)) // read to string position
							{
									...
									$pos_end = call_user_func(( $caseInsensitive ? 'stripos' : 'strpos' ),
